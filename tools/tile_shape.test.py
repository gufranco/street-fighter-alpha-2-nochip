...
class TilesTest(unittest.TestCase):
    def test_a_run_that_is_a_whole_number_of_tiles_is_cut_into_them(self):
        found = tile_shape.tiles(bytes(64))

        self.assertEqual(len(found), 2)

    def test_a_run_that_is_not_is_refused_rather_than_padded(self):
        self.assertIsNone(tile_shape.tiles(bytes(40)))

    def test_and_nothing_at_all_is_refused_too(self):
        self.assertIsNone(tile_shape.tiles(b""))

    def test_the_model_decides_what_a_tile_is_rather_than_this_file(self):
        found = tile_shape.tiles(bytes(tile_shape.TILE_BYTES))

        self.assertEqual(len(found), 1)

    def test_and_the_size_named_here_agrees_with_what_the_model_cuts(self):
...
class ReuseTest(unittest.TestCase):
    def test_a_sheet_of_identical_tiles_is_all_reuse_but_one(self):
        found = tile_shape.reuse(bytes(32 * 4))

        self.assertEqual(found, 0.75)

    def test_a_sheet_with_no_tile_twice_reuses_nothing(self):
        sheet = b"".join(bytes([n]) * 32 for n in range(4))

        self.assertEqual(tile_shape.reuse(sheet), 0.0)

    def test_a_run_that_is_not_whole_tiles_has_no_reuse_to_report(self):
        self.assertIsNone(tile_shape.reuse(bytes(40)))


class ControlTest(unittest.TestCase):
    def test_random_bytes_repeat_no_tile_at_all(self):
        found = [tile_shape.reuse(tile_shape.noise(32 * 64)) for _ in range(8)]

        self.assertEqual(max(found), 0.0)

    def test_which_is_what_makes_a_nonzero_reading_mean_something(self):
...
class AgainstTheCartridgeTest(unittest.TestCase):
    ...
    def test_most_streams_are_a_whole_number_of_tiles(self):
        found = self.measured()

        self.assertGreater(found.aligned, found.unaligned)

    def test_the_decompressed_output_repeats_tiles(self):
        self.assertGreater(self.measured().meanReuse, 0.05)

    def test_and_random_bytes_of_the_same_lengths_repeat_none(self):
        self.assertEqual(self.measured().meanReuseOfNoise, 0.0)

    def test_which_is_the_whole_claim(self):
        found = self.measured()
